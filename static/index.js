...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            if (score < 4) return '试着好好练一下？';
            if (score < 8) return 'TQL';
            if (score < 10) return 'orz';
            if (score < 12)  return '代表飞行员谢谢你';
            if (score < 15) return '谢谢卡尔';
            return '谢谢卡尔文森';
        } else {
            if (score < 32) return '试着好好练一下？';
            if (score < 64) return 'TQL';
            if (score < 96) return 'orz';
            if (score < 128) return '代表飞行员谢谢你';
            if (score < 160) return '谢谢卡尔';
            return '谢谢卡尔文森';
        }
    }
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "撞掉F35C";
        if (cookie("keyboard")) {
